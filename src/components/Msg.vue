    ...
    <div class="msg-content">
      ...
      <span class="msg-pop-container">
        <div
          ...
          :class="{ 'msg-pop-primary': role === 'user' }"
        >
          <span v-html="renderedContent" ref="contentRef"> </span>
          <!-- 复制按钮 -->
          <br />
...
const md: MarkdownIt = MarkdownIt({
  highlight: function (str: string, lang: string) {
    if (lang && hljs.getLanguage(lang)) {
      try {
        return `<div class="hl-code"><div class="hl-code-header sticky"><span>${lang}</span><button class="copy-btn">copy</button></div><div class="hljs"><code>${
    ...
    return `<div class="hl-code"><div class="hl-code-header"><span>${lang}</span><button class="copy-btn">copy</button></div><div class="hljs"><code>${md.utils.escapeHtml(
      str
    )}</code></div></div>`;
  },
});
...
const renderedContent = computed(() => {
  ...
  nextTick(() => {
    if (props.streaming) {
      ...
      existingCursors.forEach((cursor: { remove: () => any }) =>
        cursor.remove()
      );
